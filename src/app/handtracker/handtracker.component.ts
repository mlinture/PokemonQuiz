...
export class HandtrackerComponent implements OnInit {
  ...
  or slower rates
  */
  private detectedGestureSubject = new BehaviorSubject<string>('None');
  detectedGesture$ = this.detectedGestureSubject.asObservable();
  ...
  runDetection(){
    if (this.model != null){
        let predictions = this.model.detect(this.video.nativeElement).then((predictions: any) => {
            ...
            //console.log(`open hands ${openhands} closed hands ${closedhands} pointing ${pointing}`);
            ...
            else if(pointing == 1 && closedhands==0) this.detectedGesture = "Hand Pointing";
            ...
            if (openhands == 1 && closedhands == 1){ //for hint 
              this.detectedGesture = "Open and Closed Hands";
            }

            if (pointing == 1 && closedhands == 1){ //figure out later
              this.detectedGesture = "Point and Closed Hands";
            }
            this.onPrediction.emit(new PredictionEvent(this.detectedGesture));
            //this.detectedGestureSubject.next(this.detectedGesture);
